#!/usr/bin/env python3
"""
Codex runner for all DevEval repositories and configurations.
...
class CodexBatchRunner:
    """Batch runner for extensive Codex experiments across all DevEval repos, excluding TextCNN."""
    
    def __init__(self, model: str = "gpt-4.1-mini", output_dir: str = "WareHouse_codex_tb"):
        self.model = model
    ...
    def run_experiment(self, repo_path: Path, config: str) -> Dict:
        """Run a single experiment using run_codex_single.py."""
        language, repo_name = self._get_repo_info(repo_path)
        experiment_id = f"{language}-{repo_name}-{config}"
        
        try:
            # Prepare command to run single experiment
            script_path = Path(__file__).parent / "run_codex_single.py"
            cmd = [
    ...
    def run_all_experiments(self, continue_run: bool = True) -> Dict:
        ...
        print(f"\n🚀 Starting Codex batch experiments ({self.model})")
    ...
    def _save_summary(self, summary: Dict):
        ...
        summary_file = self.output_dir / f"codex_batch_summary_{timestamp}.json"
    ...
    def print_summary(self, summary: Dict):
        ...
        print(f"\n{'='*60}")
        print(f"🎯 CODEX BATCH EXPERIMENT SUMMARY")
        print(f"{'='*60}")
...
def main():
    parser = argparse.ArgumentParser(description="Run Codex batch experiments across all DevEval repositories (excludes TextCNN)")
    parser.add_argument(
        "--model",
        type=str,
    ...
    try:
        ...
        runner = CodexBatchRunner(args.model, args.output_dir)
        summary = runner.run_all_experiments(continue_run=not args.no_continue)
        runner.print_summary(summary)
        
        print(f"\n🎉 Codex batch experiments completed successfully!")
        
    except KeyboardInterrupt:
        print("\n⚠️  Interrupted by user")
        sys.exit(1)
    except Exception as e:
        print(f"\n💥 Codex batch experiments failed: {e}")
        if args.debug:
            traceback.print_exc()
        sys.exit(1)
